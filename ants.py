...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if insect.is_ant:
            if type(insect) == QueenAnt and insect.num==1:
                return
            # Phase 6: Special Handling for BodyguardAnt and QueenAnt
            #if hasattr (insect,'fakeQueen') and self.fakeQueen == False:
                #return
        ...
        if self.num==1:
            ...
            while behind is not None:
                if behind.ant is not None:
                    if behind.ant not in self.double:
                        self.double.append(behind.ant)
                        behind.ant.damage*=2
                    if behind.ant.container is True:
                        if behind.ant.ant is not None:
                            if behind.ant.ant not in self.double:
                                self.double.append(behind.ant.ant)
